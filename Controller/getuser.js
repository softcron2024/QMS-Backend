...
const getUser = asyncHandler(async (req, res) => {
    ...
    Connection.query("CALL SPscantoken(?)", [token_no], (err, response) => {
        if (err) {
            let message = "Token not found or Expired, Try again!";
            let responseCode = 0;
        ...
        if (response[0][0].ResponseCode === 0) {
            return res.status(404).json({ ResponseCode: 0, message: response[0][0].ResponseMessage });
        }

        return res.status(200).json({ ResponseCode: 1, message: "Your token is valid" });
    });
});
